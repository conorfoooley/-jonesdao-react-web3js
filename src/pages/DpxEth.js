...
const DpxEth = () => {
    ...
    const lockText = "Deposits and withdrawals are held throughout an epoch to retain proportional option sizing within an epoch. When depositing during a live epoch, your capital will not be deployed until the next epoch. If you wish to sell or withdraw your assets, the sell is honored at the time a withdrawal is initiated, but you cannot access your claimable assets until the conclusion of the epoch. UTC unlock times may vary based on Dopex epoch timings. Learn More";
    ...
    const withDrowText = "Once user funds have been implemented in a vault strategy they cannot be withdrawn until the vault closes its position the following Friday (UTC times may vary).<br/>Users can signal to withdraw during an active epoch but must wait until the epoch concludes to claim. Users have the ability to withdraw funds instantly during the layover between epochs.<br/>This decision is cancelable, with funds remaining deployed in vault strategies if indicated.";
    const incentivesText = "The vault incentive structure consists of a 10% performance incentive. If the weekly strategy is profitable, the weekly performance incentive is retained on the premiums earned. If the weekly strategy is unprofitable, there are no incentives retained.<br/>If the weekly strategy is unprofitable, there are no incentives are retained.";
    ...
    function createGradient(ctx, area) {
        const colorStart = '#191c1d';
        const colorMid = '#2b4851';
        const colorEnd = '#3e7789';
    ...
    return (
        <div className='dpxEth'>
            <Header/>
            <div className="dpxEth-child">
            <Container>
                <Row className="mt-4">
